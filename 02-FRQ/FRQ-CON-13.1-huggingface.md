# FRQ-CON-13.1 — Konekcija prema HuggingFace pod-sustavu
...
| **Sestrinski** | [`FRQ-DRV-13`](FRQ-DRV-13-llm-model.md) (operacije nad modelom) · [`FRQ-CON-13.2`](FRQ-CON-13.2-remote-model.md) (ostali dobavljači — po staroj osi, `DR-PRC-001` §Cijena) |
...
driver preda knjižnici kao HTTP pozadinu (`FRQ-DRV-13` §5 t.2).
...
4. A4 u `_configure_service` potvrđuje da je **spremište upotrebljivo** (postoji ili se 
   može stvoriti, i direktorij je). Postoji li *imenovani model*, pitanje je za driver
   (`FRQ-DRV-13` §5 t.3) — konekcija daje pristup, ne odgovara na pitanja o sadržaju.
...
kvarovi pripadaju driveru (`FRQ-DRV-13` §6). Kvarovi ove konekcije su lokalni i konfiguracijski:
...
| `token` se ne razrješava | kvar konfiguracije; vrijednost se **ne** ispisuje u zapis (`NFRQ-SEC-06`) |
| `offline: true`, a model nedostaje | nije kvar konekcije — driver odbija dohvat (`BR-08`, `FRQ-DRV-13` §6) |
...
| uvoz | test maskiranja (`huggingface_hub`, `transformers`, `tqdm`) | modul se uvozi (`NFRQ-SEC-03`) |
...
|---|---|
| `NFRQ-SEC-01` | jedna konekcija po pod-sustavu; kompromitacija HF pristupa ne doseže druge dobavljače |
| `NFRQ-SEC-02` | nasljeđivanje donosi i roditeljske ključeve — konfiguracijska površina raste; prihvaćena cijena (`DR-PRC-001` §Cijena) |
| `NFRQ-SEC-03` | `huggingface_hub` je lazy uvoz; modul se uvozi i bez njega |
| `NFRQ-SEC-06` | `token` se maskira u zapisu (naslijeđeni `_safe_log_kwargs`); zapisuje se samo *je li* autentificirano |
| `NFRQ-ORG-08` | razrješavanje cachea ne prepisuje se — koristi `DownloadedModels` |
...
2. **Definicija „valjanog modela"** je heuristika u driveru (config + težine); mjesto joj je
   ovdje ili u `FRQ-DRV-13`, uz sign-off.
3. `FRQ-CON-13.2` je i dalje pisan po osi lokalno/mrežno — treba ga prepisati po pod-sustavima
   (`DR-PRC-001` §Cijena).
