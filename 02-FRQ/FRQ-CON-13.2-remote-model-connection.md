# FR-CON-13.2 — Mrežna konekcija prema jezičnom modelu (više dobavljača)
...
| **Sestrinski** | [`FR-CON-13.1`](FRQ-CON-13.1-huggingface-connection.md) (HuggingFace) · [`FR-DRV-13`](FRQ-DRV-13-llm-model.md) (operacije nad modelom) |
...
potvrđen** — dohvat i inferencija su `FR-DRV-13`.
...
| 3 | propusnost je beskonačna | — dohvat sadržaja nije posao konekcije | konekcija ne prenosi model (`FR-DRV-13`) |
...
|---|---|
| `NFR-SEC-01` | **konekcija po dobavljaču**, ne jedna za sve: kompromitacija jednog pristupa ne smije dosegnuti ostale |
| `NFR-SEC-02` | registar dobavljača (A5) je unutarnji; javno sučelje ostaje `connect` / `disconnect` + stanje |
| `NFR-SEC-03` | provenijencija je **nejednaka po dobavljaču**: pinanje revizije postoji kod HF-a i Vertexa, drugdje ne. Jamstvo se deklarira po dobavljaču, ne obećava jednako svima (§11 t.4) |
| `NFR-SEC-06` | pristupni podatak ne ulazi ni u zapis ni u poruku greške (§8 k.4); kvota i endpoint smiju |
...
3. **Poslužitelj na istom stroju** (Ollama/vLLM/TGI) vođen je ovdje jer ima endpoint; potvrditi
   da je to željena granica prema [`FR-CON-13.1`](FRQ-CON-13.1-huggingface-connection.md).
