...
### 2.3 Uvedene helper klase (NFR-ORG-02 usklađene)
...
| NFR-ORG-01 t.4 | dijeljeni helper imenovan po **sposobnosti** (`routing`), ne po sloju |
| NFR-ORG-02 | kvalificirana, uloga-eksplicitna imena; akronimi velikim slovom |
...
**Negativno / rizik**
- **Per-class fan-in (NFR-ORG-01 kriterij 1).** `RoutingRule`/`PatternSpec` trenutno koristi samo `processors`, `*DestinationRouter` samo `strategies`. Zajednički `ROUTE_KEY` + apstraktni ugovor su genuino ≥2 domene i sidre modul u dijeljeni `helpers/` (izbjegavaju processors↔strategies brid). Kad druga domena (npr. `drivers`) usvoji router, kriterij 1 je zadovoljen i za konkrete. Do tada: prihvaćeni WARN (tolerancija).
