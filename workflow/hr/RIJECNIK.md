...
| **Srodni artefakti** | `dictionary.json` (kontrolirani vokabular koda), `documentation/LITERATURE.md` (ključevi 1–52) |
...
značenja za ljudski jezik doktrine, kao što `dictionary.json` fiksira
...
**Registar (dictionary.json)** — strojno čitljivi kontrolirani vokabular
koda: domene, opseg, obitelji baza, uloge TypeVarova; TARGET vokabular čija
odstupanja lint prijavljuje. `[artefakt/kriterij | metoda | FR.md | NFR.md]`
...
1. **Izvor istine:** ovaj dokument je autoritativan za doktrinarni diskurs;
   `dictionary.json` za identifikatore koda; `documentation/literature.md` za
   reference. Tri registra, tri domene — pojmovi koji postoje u više domena
   (npr. Document) povezuju se, ne dupliciraju.
