...
## Bilješka o sintezi (v0.4 → v0.4.1)

1. **Nazivlje dosljedno DR**: preimenovanje ADR → DR (autorova izmjena u
   naslovu sekcije) provedeno kroz cijelo tijelo dokumenta; naslijeđeni
   paket `ADR-001…014` vodi se kao `DR-COR-001…014`, a serija dobiva prefiks
   po projektu (2026-07-28); dodana poveznica na METHODOLOGY §8.1 (format
   zapisa je zamjenjiva pod-metoda).
2. **Sekcija Reference zamjenjuje tablicu literature**: uklonjena lokalna
   tablica, zalutali fragment lijepljenja (uključujući uredničku uputu
   koja je dospjela u tekst) i duplicirane Napomene (žive uz izvor istine
   u documentation/LITERATURE.md).
3. **Uveden sloj P-oznaka**: gdje tvrdnja postoji kao postulat, tekst
   citira P-NN (registar postulata) umjesto izravnog ključa literature —
   arhitektura referenciranja: filozofija → postulati → literatura.
   Izravni ključevi [n] ostaju gdje postulata nema (genealogija DR prakse
   [44][45][46]).
4. **Odlučen standard citiranja**: numerički ključevi (IEEE stil) uz
   append-only pravilo; Harvard odbačen zbog migracijskog troška i rizika
   transkripcijskog gubitka; stabilni identifikatori (literatura.yaml) kao
   evolucijski put (DR kandidat).
5. Povijesna točnost Bilješke v0.3→v0.4 vraćena (DR-WFL-002).

## Bilješka o sintezi (v0.3 → v0.4)

Za analizu autora — intervencije u odnosu na v0.3, redom težine:

1. **Nova sekcija Hipoteze** (H1–H3, opovrgljivi oblici) — provodi doktrinu
   mjerljivosti na samoj filozofiji; bez nje bi doktrina kritičkog
   razmišljanja bila ranjiva na vlastiti test.
2. **Razriješen „ili Obrnuto"**: iteracija arhitektura ⇄ implementacija
   dopuštena isključivo pod kaskadom, revizija samo kroz DR; uklonjena i
   hedgirana rečenica o nenametanju ontološkog diskursa.
3. **Zero-trust razdvojen po slojevima**: epistemička jezgra („povjerenje
   se dokazuje") na razini filozofije; imenovana sigurnosna politika sloj
   niže.
4. **DR numeracija fiksirana**: sekvencijalna, postojeći paket normativan
   (v0.3 je implicirala kategorijsku kroz DR-WFL-002).
5. Dodane reference (ključevi na konsolidiranu tablicu) i genealogija ADR
   prakse; sekcija Motivacija dobila povijesno utemeljenje [40][41][47][48].
6. Sekcija o publikama dokumentacije (Ashby argument) ušla u *Arhitektura
   kao obrazovni artefakt*.
7. Jezična redaktura (tipfeleri, dvostruki naslovi, konsolidacija
   ponovljenih odlomaka); sadržajno ništa nije uklonjeno, samo spojeno.