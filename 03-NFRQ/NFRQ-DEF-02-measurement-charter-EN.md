...
3. **The graph includes every edge** — static, relative and dynamic (`ClassLoader`, YAML) — and
   declares its blind spots (as with the fan-in lower bound in `NFRQ-ORG-01`).
...
`NFRQ-ORG-08` c.3 · `NFRQ-OBS-03` c.5 · `NFRQ-SEC-01` c.1–2 · `NFRQ-SEC-02` c.1, c.3 ·
`NFRQ-SEC-03` c.6 · `NFRQ-SEC-04` c.2 · `NFRQ-SEC-05` c.2.
