# NFR-ORG-08 — Deduplication: one place per rule, not per shape
...
> **Why 08 and not 07.** `NFR-ORG-07` (input surface / `ALLOWED`) is cited by
> [`NFR-SEC-06`](NFR-SEC-06-audit-confidentiality-EN.md) c.1 and **measured by the lint as an
...
2. A procedure repeated **under the same contract** is extracted into a helper or mixin **of the
   domain it belongs to** ([`NFR-ORG-01`](NFR-ORG-01-helper-locality-EN.md)), not into a generic
   `Helper` ([`NFR-ORG-02`](NFR-ORG-02-class-nomenclature-EN.md)).
3. **`[M]`** The count of repeated blocks above a similarity threshold is a **diagnostic, not a
   ...
   [`NFR-DEF-02`](NFR-DEF-02-measurement-charter-EN.md).
...
| MDL mirror ([`NFR-SEC-02`](NFR-SEC-02-attack-surface-EN.md)) | `L(code) + L(abstraction)`: an abstraction hiding differences grows faster than the saving |
